...
void GroupItem::addToGroup(QGraphicsItem *item)
{
    if (!item)
    {
        ...
        item->setPos(mapFromItem(item, 0, 0));//this->mapFromItem(item, 0, 0)
        ...
        QMatrix4x4 m;//transformList = NULL
...
{
    qreal halfpw = (pen().widthF() + BaseItem::resizeHandleWidth+100) / 2 ;
